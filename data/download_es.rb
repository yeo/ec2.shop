...
[{
  name: 'es-ondemand',
  url: "https://b0.p.awsstatic.com/pricing/2.0/meteredUnitMaps/es/USD/current/es-ondemand.json?timestamp=#{ts}"
}].each do |instance_class|
  puts "#{instance_class[:name]} downloading..."

  region_price_data = HTTPX.get(instance_class[:url])
  File.write("data/es/#{instance_class[:name]}.json", region_price_data)
end
...
regions.each do |key, region|
  ...
  %w(1%20year 3%20year).each do |y|
    %w(No%20Upfront Partial%20Upfront All%20Upfront).each do |p|
      %w(General%20purpose Compute%20optimized Memory%20optimized Storage%20optimized OR1).each do |t|
        url = "https://b0.p.awsstatic.com/pricing/2.0/meteredUnitMaps/es/USD/current/es-reservedinstance/#{y}/#{p}/#{rn}/#{t}/index.json?timestamp=#{ts}"

        puts "fetch #{url}"
        region_price_data = HTTPX.get url

        File.write("data/es/es-reservedinstance-#{y}-#{p}-#{rc}-#{t}.json", region_price_data)
      end
    end
  end
end
